...
import { RoadModal } from "./RoadModal.jsx";

export const Roads = () => {
  const { roads, loading, getRoads, changeStatus } = useRoadsStore();
  ...
  useEffect(() => {
    getRoads({ status: statusFilter, typeRoad: typeFilter });
  }, [statusFilter, typeFilter, getRoads]);
  ...
  const handleToggleStatus = async (road) => {
    const nextStatus = road.status === "ACTIVE" ? "INACTIVE" : "ACTIVE";
    await changeStatus(road._id || road.routeCode, nextStatus);
  };
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          <h1 className="text-2xl font-bold text-main-blue">Gestión de Rutas</h1>
          <p className="text-gray-500 text-sm">Administración de líneas troncales, expresas y alimentadoras</p>
        </div>
      ...
      <div className="bg-white rounded-xl shadow-xs border border-pale-blue/30 p-4 flex flex-wrap gap-4 items-center justify-between">
        <div className="flex flex-wrap gap-3 items-center">
          ...
          >
            ...
            <option value="RELEVOS">RELEVOS (TuBus/Transurbano)</option>
          </select>
        </div>

        <span className="text-xs text-gray-500 font-medium">
          Total: {roads.length} rutas encontradas
        </span>
      </div>
            ...
            <table className="w-full text-left border-collapse">
              <thead>
                <tr className="bg-gray-50 border-b border-pale-blue/20 text-xs font-bold text-main-blue uppercase">
                  ...
                  <th className="px-6 py-3">Puntos LineString</th>
              ...
              <tbody className="divide-y divide-gray-100 text-sm">
                {roads.map((road) => (
                  <tr key={road._id || road.routeCode} className="hover:bg-light-green/10 transition-colors">
                    ...
                    <td className="px-6 py-4 text-xs text-gray-500">
                      {road.path?.coordinates?.length || 0} puntos
                    </td>
                    <td className="px-6 py-4">{getStatusBadge(road.status)}</td>
